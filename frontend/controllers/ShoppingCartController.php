...
class ShoppingCartController extends Controller
{
    ...
    public function actionAddToShoppingCart($product_id)
    {
        ...
        if (Yii::$app->request->isAjax) {
            $exist = ShoppingCart::find()->where(['product_id' => $product_id, 'user_id' => $user_id])->one();
            $product = Product::find()->select('price')->where(['idProd' => $product_id])->one();
            if (!empty($exist)) {
                $product_price = $product->price;
                $exist->quantity =  $exist->quantity + 1;
                $exist->price = $exist->quantity * $product_price;
                $exist->save();
                return Json::encode(['success' => 'product quantity updated in shopping cart']);
            } else {
                ...
                $shopping_cart->save();
                return Json::encode(['success' => 'product quantity added in shopping cart']);
            }
        }
    }
